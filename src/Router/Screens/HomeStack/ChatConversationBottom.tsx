...
const ChatConversationBottom = () => {
  ...
  const getChannelRoom = useMemo(() => {
    const channelRoom = channels.find(
      (chan) =>
        chan.topic.split(':')[1] === 'room' + actualRoom?.room.toString()!
    );
  ...
  const onSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      if (getChannelRoom !== null && !!message?.is_blocked === false) {
        getChannelRoom.send({
          type: 'broadcast',
  ...
  const sendImage = async (image: ImagePicker.ImageInfo) => {
    ...
    try {
      ...
      let { error: uploadError, data: imageData } = await supabase.storage
        .from('users-images')
