...
const ChatConversationHeader = () => {
  ...
  const actualRoom = rooms.find(
    (roomState) => roomState.room === route.params?.room_id
  );
  ...
  const isUserOnline = useMemo(() => {
    const isOnline = Object.keys(onlineUsers).find(
      (userId) => userId === userToChat?.id
    );
            ...
            >
              {userToChat?.username &&
                userToChat?.username[0].toUpperCase() +
                  userToChat?.username[1].toUpperCase()}
            </Avatar>
            <Pressable
              _pressed={{
              ...
              {!isUserBlocked.isRoomBlocked && !userIsTypying ? (
                <HStack space='1' alignItems={'center'}>
                  <Box
                    ...
                    width='2'
                    borderRadius={'full'}
                    bg={isUserOnline ? 'green.500' : 'red.500'}
                  />
